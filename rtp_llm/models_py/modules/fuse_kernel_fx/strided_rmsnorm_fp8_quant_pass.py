...
def strided_rmsnorm_consumer(
    ...
    """Consumer-token: if ``contig`` has a registered strided source, run the fused
    strided rmsnorm into ``output``; otherwise fall back to the baseline rmsnorm.
    """
    from rtp_llm.models_py.modules.fuse_kernel_fx.quant_provenance import (
        lookup_strided,
    )

    strided = lookup_strided(contig)
    if strided is not None:
        try:
            fused = fused_strided_rmsnorm(strided, weight, float(eps))
            output.copy_(fused)
            return
        except Exception as exc:  # noqa: BLE001
            logger.warning(
                "GraphFX strided_rmsnorm consumer fallback: %s", exc
            )
...
def _rewrite_producer_outer(
    gm: torch.fx.GraphModule, node: torch.fx.Node
) -> bool:
    """Wrap ``contig = strided_view.contiguous()`` with a token that remembers the
    strided source. Only fires when the contig only flows to the graph output (so
    the rmsnorm is in a downstream subgraph)."""
    ...
    with gm.graph.inserting_after(node):
        token = gm.graph.call_function(strided_producer_token, args=(node, src))
