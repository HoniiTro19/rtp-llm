...
def ensure_graphfx_fusions_registered() -> None:
    ...
    install_compile_friendly_fused_qk_rope_cat_cache_mla_wrapper()
    install_compile_friendly_fused_qk_rope_quant_wrapper()
...
# ------------------------- fused_qk_rope_cat_cache_mla custom op (sparse MLA)

_FUSED_QK_ROPE_CAT_CACHE_MLA_WRAPPER_INSTALLED = False
_FUSED_QK_ROPE_CAT_CACHE_MLA_ORIGINAL = None
_FUSED_QK_ROPE_CAT_CACHE_MLA_CUSTOM_OP = None


def _build_fused_qk_rope_cat_cache_mla_custom_op():
    global _FUSED_QK_ROPE_CAT_CACHE_MLA_CUSTOM_OP
    if _FUSED_QK_ROPE_CAT_CACHE_MLA_CUSTOM_OP is not None:
        return _FUSED_QK_ROPE_CAT_CACHE_MLA_CUSTOM_OP
    if _FUSED_QK_ROPE_CAT_CACHE_MLA_ORIGINAL is None:
        return None
    try:
        import torch
        ...
        def _impl(
            ...
            _FUSED_QK_ROPE_CAT_CACHE_MLA_ORIGINAL(
                q=q,
        ...
        op = torch.library.custom_op(
            "rtp_llm_graphfx::fused_qk_rope_cat_cache_mla",
            _impl,
            mutates_args=("q", "k_pe", "kv_cache"),
        )
        op.register_fake(_fake)
        _FUSED_QK_ROPE_CAT_CACHE_MLA_CUSTOM_OP = getattr(op, "_opoverload", op)
        return _FUSED_QK_ROPE_CAT_CACHE_MLA_CUSTOM_OP
    except Exception as exc:
        logger.warning(
            "GraphFX fused_qk_rope_cat_cache_mla custom op build failed: %s", exc
        )
        return None


def install_compile_friendly_fused_qk_rope_cat_cache_mla_wrapper() -> None:
    """Wrap ``fused_qk_rope_cat_cache_mla`` (sparse-MLA RoPE+cat+cache) as a
    ``torch.library.custom_op``.

    The underlying triton kernel mutates ``q`` / ``k_pe`` / ``kv_cache`` via
    the ``triton_kernel_wrapper_mutation`` HigherOrderOperator that Dynamo
    cannot fakify with our input types.  Wrapping it as a custom op with
    ``mutates_args=("q", "k_pe", "kv_cache")`` keeps the contract visible
    in FX while short-circuiting fake-tensor execution to a no-op.

    After this hook runs the call site in ``SparseMlaImpl.forward`` can be
    traced — the FX subgraph contains a single ``rtp_llm_graphfx::
    fused_qk_rope_cat_cache_mla`` opaque leaf node instead of trying to
    trace into the kernel launch.
    """
    global _FUSED_QK_ROPE_CAT_CACHE_MLA_WRAPPER_INSTALLED
    global _FUSED_QK_ROPE_CAT_CACHE_MLA_ORIGINAL
    if _FUSED_QK_ROPE_CAT_CACHE_MLA_WRAPPER_INSTALLED:
        return
    if not env_flag("ENABLE_GRAPHFX_FUSION"):
        return
    try:
        from rtp_llm.models_py.triton_kernels.sparse_mla import (
            fused_qk_rope_cat_cache_mla as _module,
        )

        original = getattr(_module, "fused_qk_rope_cat_cache_mla", None)
        if original is None:
            return
        if getattr(original, "_graphfx_wrapped", False):
            _FUSED_QK_ROPE_CAT_CACHE_MLA_WRAPPER_INSTALLED = True
            return
        _FUSED_QK_ROPE_CAT_CACHE_MLA_ORIGINAL = original
        custom_op = _build_fused_qk_rope_cat_cache_mla_custom_op()
        ...
        def _compile_friendly(
            ...
            if _is_fake_or_meta_tensor(q) or _is_fake_or_meta_tensor(kv_cache):
                return None
            return custom_op(
                q,
                compressed_kv,
        ...
        _compile_friendly.__name__ = "fused_qk_rope_cat_cache_mla"
        _compile_friendly.__qualname__ = "fused_qk_rope_cat_cache_mla"
        try:
            setattr(_compile_friendly, "_graphfx_wrapped", True)
            setattr(_compile_friendly, "_graphfx_original", original)
        except Exception:
            pass
        # Patch the module attribute so direct import of the module fetches
        # the wrapper.
        try:
            setattr(_module, "fused_qk_rope_cat_cache_mla", _compile_friendly)
        except Exception:
            pass
        # Refresh pre-existing imports in modules that already cached the
        # original (most importantly ``flashmla_sparse_impl``).
        for module_name, module in list(sys.modules.items()):
            if getattr(module, "fused_qk_rope_cat_cache_mla", None) is original:
                try:
                    setattr(module, "fused_qk_rope_cat_cache_mla", _compile_friendly)
                except Exception:
                    pass
        ...
        _FUSED_QK_ROPE_CAT_CACHE_MLA_WRAPPER_INSTALLED = True
        logger.info(
            "GraphFX installed compile-friendly fused_qk_rope_cat_cache_mla wrapper"
        )
    except Exception as exc:
        logger.warning(
            "GraphFX: compile-friendly fused_qk_rope_cat_cache_mla install failed: %s",
            exc,
        )


# ------------------------- fused_qk_rope_quant custom op (sparse MLA, indexer Q)

_FUSED_QK_ROPE_QUANT_WRAPPER_INSTALLED = False
_FUSED_QK_ROPE_QUANT_ORIGINAL = None
_FUSED_QK_ROPE_QUANT_CUSTOM_OP = None


def _build_fused_qk_rope_quant_custom_op():
    global _FUSED_QK_ROPE_QUANT_CUSTOM_OP
    if _FUSED_QK_ROPE_QUANT_CUSTOM_OP is not None:
        return _FUSED_QK_ROPE_QUANT_CUSTOM_OP
    if _FUSED_QK_ROPE_QUANT_ORIGINAL is None:
        return None
    try:
        import torch
        ...
        def _impl(
            ...
            return _FUSED_QK_ROPE_QUANT_ORIGINAL(
                q,
                k,
                positions,
                cos_sin_cache,
        ...
        def _fake(
            ...
            q_fp8 = torch.empty(
                (num_tokens, int(index_n_heads), int(index_head_dim)),
                dtype=torch.float8_e4m3fn,
                device=q.device,
            )
            q_scale = torch.empty(
                (num_tokens, int(index_n_heads)),
                dtype=torch.float32,
                device=q.device,
            )
            k_out = torch.empty(
                (num_tokens, int(index_head_dim)),
        ...
        _fake.__annotations__ = annotations
        op = torch.library.custom_op(
            "rtp_llm_graphfx::fused_qk_rope_quant",
            _impl,
            mutates_args=(),
        )
        op.register_fake(_fake)
        _FUSED_QK_ROPE_QUANT_CUSTOM_OP = getattr(op, "_opoverload", op)
        return _FUSED_QK_ROPE_QUANT_CUSTOM_OP
    except Exception as exc:
        logger.warning(
            "GraphFX fused_qk_rope_quant custom op build failed: %s", exc
        )
        return None


def install_compile_friendly_fused_qk_rope_quant_wrapper() -> None:
    """Wrap ``fused_qk_rope_quant`` (indexer Q+K rope+had+quant) as a
    ``torch.library.custom_op``.

    Returns ``(q_fp8, q_scale, k_out)`` as 3 fresh tensors (no in-place
    mutation of inputs).  Wrapping keeps the kernel opaque to Dynamo's
    fakification machinery so the call appears as a single FX node in
    ``Indexer._fused_forward_decode``'s sub-compile boundary.
    """
    global _FUSED_QK_ROPE_QUANT_WRAPPER_INSTALLED, _FUSED_QK_ROPE_QUANT_ORIGINAL
    if _FUSED_QK_ROPE_QUANT_WRAPPER_INSTALLED:
        return
    if not env_flag("ENABLE_GRAPHFX_FUSION"):
        return
    try:
        from rtp_llm.models_py.triton_kernels.sparse_mla import (
            fused_q_rope_quant as _module,
        )

        original = getattr(_module, "fused_qk_rope_quant", None)
        if original is None:
            return
        if getattr(original, "_graphfx_wrapped", False):
            _FUSED_QK_ROPE_QUANT_WRAPPER_INSTALLED = True
            return
        _FUSED_QK_ROPE_QUANT_ORIGINAL = original
        custom_op = _build_fused_qk_rope_quant_custom_op()
        if custom_op is None:
            return
        ...
        def _compile_friendly(
            ...
            if _is_fake_or_meta_tensor(q) or _is_fake_or_meta_tensor(k):
                # Under FakeTensor (Dynamo trace) the custom_op's register_fake
                # impl is invoked instead.
                return custom_op(
                    q,
                    k,
                    positions,
        ...
        _compile_friendly.__name__ = "fused_qk_rope_quant"
        _compile_friendly.__qualname__ = "fused_qk_rope_quant"
        try:
            setattr(_compile_friendly, "_graphfx_wrapped", True)
            setattr(_compile_friendly, "_graphfx_original", original)
        except Exception:
            pass
        try:
            setattr(_module, "fused_qk_rope_quant", _compile_friendly)
        except Exception:
            pass
        for module_name, module in list(sys.modules.items()):
            if getattr(module, "fused_qk_rope_quant", None) is original:
                try:
                    setattr(module, "fused_qk_rope_quant", _compile_friendly)
                except Exception:
                    pass
        _dynamo_allow_in_graph(custom_op)
        _dynamo_allow_in_graph(_compile_friendly)
        _FUSED_QK_ROPE_QUANT_WRAPPER_INSTALLED = True
        logger.info("GraphFX installed compile-friendly fused_qk_rope_quant wrapper")
    except Exception as exc:
        logger.warning(
            "GraphFX: compile-friendly fused_qk_rope_quant install failed: %s", exc
        )
...
def install_graphfx_compile_disable_wrappers() -> None:
    ...
    # ``SparseMlaImpl.forward`` is left traceable now that
    # ``fused_qk_rope_cat_cache_mla`` is wrapped as a torch.library.custom_op
    # (see ``install_compile_friendly_fused_qk_rope_cat_cache_mla_wrapper``).
    # The custom_op is fakifiable, so Dynamo treats the call as an opaque
    # leaf node and the kernel call appears in the FX subgraph rather than
    # being hidden inside compile-disabled code.
    installed += _install_compile_disable_methods(
...
def _allow_graphfx_fusion_candidates_in_graph() -> None:
    ...
    # Indexer fused kernel + strided consumer/producer tokens.
    try:
        from rtp_llm.models_py.modules.fuse_kernel_fx.quant_provenance import (
            lookup_strided,
            remember_strided,
        )
        from rtp_llm.models_py.modules.fuse_kernel_fx.strided_rmsnorm_fp8_quant_pass import (
            strided_producer_token,
            strided_rmsnorm_consumer,
        )
        ...
        logger.warning(
            "GraphFX allow_in_graph indexer/strided registration failed: %s", exc
        )
    # New fuse kernels from cherry-pick (rope_had_quant + rope_cache_mla):
    # without allow_in_graph Dynamo would graph-break at these calls and
    # the fuse kernels would never appear in any FX subgraph.
    try:
        ...
        for fn in (fused_qk_rope_cat_cache_mla, fused_qk_rope_quant):
            try:
                _dynamo_allow_in_graph(fn)
            except Exception:
                continue
        # The pybind ``rtp_llm_ops.indexer_k_quant_and_cache`` writes the
        # quantised K straight into the paged KV cache; Dynamo treats it as
        # an opaque mutating leaf — allow it so the
        # ``Indexer._fused_forward_decode`` sub-graph can trace through.
        try:
            from rtp_llm.ops.compute_ops import rtp_llm_ops as _rtp_llm_ops_for_indexer

            _kq = getattr(_rtp_llm_ops_for_indexer, "indexer_k_quant_and_cache", None)
            if _kq is not None:
                _dynamo_allow_in_graph(_kq)
        except Exception:
            pass
    except Exception as exc:
        logger.warning(
            "GraphFX allow_in_graph fused_qk_rope_cat_cache_mla failed: %s", exc
        )
...
_MUTATING_LEAF_TARGETS = {
    ...
    "rmsnorm_mutating",
    "rtp_llm_graphfx::rmsnorm_mutating",
    # GraphFX cross-graph consumer helpers (mutate the output buffer in place).
    "strided_rmsnorm_consumer",
    "strided_producer_token",
    # MoE shared-expert sigmoid_gate_add (mutates the experts buffer in place).
    "sigmoid_gate_scale_add_triton",
    # Sparse-MLA fused kernels — mutate q_pe / k_pe / kv_cache in place.
    "fused_qk_rope_cat_cache_mla",
    "fused_qk_rope_quant",
