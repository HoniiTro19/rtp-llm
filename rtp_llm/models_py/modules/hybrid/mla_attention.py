...
from rtp_llm.models_py.modules.fuse_kernel_fx.quant_provenance import remember_strided
...
# CUDA-only fused strided RMSNorm (used by the bit-exact kv_a_norm path
# preserved in 864063700; the q_a_norm path is intentionally NOT fused
# eagerly because of 1-ULP Triton vs C++ precision drift on H=2048).
...
def _strided_contig(strided: torch.Tensor) -> torch.Tensor:
    """``.contiguous()`` + register the strided source in the GraphFX provenance
    registry so the downstream rmsnorm sub-FX-graph (which only sees the
    contiguous result as a placeholder, due to a Dynamo graph break around
    ``torch.cuda.current_stream().cuda_stream``) can recover the strided source
    and dispatch to ``fused_strided_rmsnorm`` instead of the
    contiguous-then-rmsnorm baseline.

    Semantically a pure ``.contiguous()`` — no value-level change.  The
    registration is no-op when the strided pass is disabled or the consumer
    never looks up the entry.

    Marked ``@torch.compiler.disable`` so Dynamo treats it as an opaque op and
    does not optimise away the ``.contiguous()`` call (which would lose us the
    strided source pointer).
    """
    contig = strided.contiguous()
    remember_strided(contig, strided)
    return contig
...
class MlaAttention(nn.Module):
    ...
    def __init__(
        ...
        # MLA strided RMSNorm fusion is delegated to GraphFX
        # (``strided_rmsnorm_fp8_quant_pass`` + ``_strided_contig`` helper).
        # No eager flag here — the FX consumer-mode rewrites the contig+norm
        # chain to ``fused_strided_rmsnorm`` at trace time.
    ...
    def forward(
        ...
        if self.q_lora_rank > 0:
            ...
            q_c = self.q_a_layernorm(_strided_contig(q))
            q = self.q_b_proj(q_c)
        else:
        ...
        compressed_kv = self.kv_a_layernorm(_strided_contig(compressed_kv))
