...
class TestFusedQRopeQuant(TestCase):
    ...
    def test_fused_matches_reference_neox(self):
        ...
        q_fp8_ref, q_scale_ref = self._reference_rope_quant(
            q.clone(), positions, cos_sin_cache, n_heads, head_dim, rot_dim,
            is_neox_style=True,
        )
    ...
    def test_fused_zero_tokens(self):
        ...
        q_fp8, q_scale = fused_q_rope_quant(
            q, positions, cos_sin_cache, n_heads, head_dim, rot_dim,
        )
...
class TestIndexTransformTwoStep(TestCase):
    ...
    def test_triton_convert_produces_valid_physical_addresses(self):
        ...
        for b in range(batch_size):
            block_table[b] = torch.randperm(num_pages, device=device).to(torch.int32) + b * 1000
    ...
    def test_two_step_matches_direct_lookup(self):
        ...
        block_table = torch.zeros(batch_size, num_pages, dtype=torch.int32, device=device)
        for b in range(batch_size):
            block_table[b] = torch.randperm(num_pages, device=device).to(torch.int32) + b * 500
...
class TestPerformance(TestCase):
    ...
    def test_fused_q_rope_quant_speedup(self):
        ...
        op = IndexerOp(
            index_n_heads=n_heads, index_head_dim=head_dim, index_topk=2048,
            rope_head_dim=rot_dim, cos_sin_cache=cos_sin_cache,
            blocksize=64, block_size=128, scale_fmt="ue8m0",
            is_neox_style=True, skip_hadamard=True,
        )
