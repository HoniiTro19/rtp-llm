...
# FP8 KV slot byte layouts. See _swa_fp8_pack.py module docstring and
# decode/fp8_kv_quant_decode_op.py for the SWA/CSA/HCA 584B layout;
# _compressor_fused_triton.py for the indexer 132B layout. plan §1.1.
DSV4_FP8_SLOT_BYTES = 584  # SWA / CSA / HCA: fp8[512] + pad + fp32 scale
...
class PoolBackedModule(nn.Module):
    ...
    def _bind_kv_cache_from_pool(
        ...
        # TEMP fp8 hack (plan §1.1): dequant CSA/HCA/SWA 584B slots → bf16.
        if is_fp8_swa_slot_pool(self._kv_pool_view) and D == 512:
            from rtp_llm.models_py.modules.dsv4._swa_fp8_pack import unpack_swa_fp8

            gathered = unpack_swa_fp8(gathered).to(dtype)
    ...
    def _scatter_kv_cache_to_pool(
        ...
        # TEMP fp8 hack (plan §1.1): pack bf16 → 584B slots for CSA/HCA/SWA.
        if is_fp8_swa_slot_pool(self._kv_pool_view) and D == 512:
            from rtp_llm.models_py.modules.dsv4._swa_fp8_pack import pack_swa_fp8

            flat = pack_swa_fp8(flat.to(torch.bfloat16))
