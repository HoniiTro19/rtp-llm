...
class Compressor(PoolBackedModule):
    ...
    def forward_decode_vectorized(
        ...
        dtype = x.dtype
        device = x.device
        # PP4: when bound pool is FP8 packed (132B/slot uint8) AND the env
        # gate is on, take the fused {pool→RMSNorm→RoPE→FP8 quant→scatter}
        # fast path which writes the cache directly. State buffers still
        # need their normal pool bind; the kv_cache materialization is
        # skipped (cache=None makes the scatter step a no-op).
        use_fused = self._fp8_pool_active() and _fused_compressor_enabled()
        ...
        try:
            ...
            if use_fused:
                ...
                # scatter into the indexer's packed cache. Skips the bf16
                # kv_compressed materialization & per-batch index_put scatter.
                ...
                self._run_fused_cache_write(
                    kv_state_3d, score_state_3d, slots, rope_idx
                )
    ...
    def _forward_prefill_body(
        ...
        if use_fused:
            ...
            self._run_fused_cache_write(
                kv_flat, score_flat, slots, freq_idx, overlap_flag=False
            )
            return None
