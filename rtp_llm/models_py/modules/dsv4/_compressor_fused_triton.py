...
def _v4_compressor_fused_kernel(
    ...
    # ── FP8 quant (single block, head_dim==QUANT_BLOCK).  Match
    # ``quantize_indexer_k`` in _indexer_fp8_quant_triton.py byte-for-byte:
    #   scale = max(absmax/FP8_MAX, 1e-12)
    #   q_fp8 = round_e4m3(rotated / scale)            (fp32 → fp8e4nv)
    # plus a bf16 round-trip on the input to mirror downstream numerics
    # (pooled+norm+rope are otherwise fp32 throughout).
    rotated_bf = rotated.to(tl.bfloat16).to(tl.float32)
    abs_v = tl.where(d_mask, tl.abs(rotated_bf), 0.0)
    absmax = tl.max(abs_v, axis=0)
    scale = tl.maximum(absmax * (1.0 / FP8_MAX), 1e-12)
    q_f32 = rotated_bf / scale
