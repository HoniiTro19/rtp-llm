...
class LayerwiseQKRMSNormTPTest(TestCase):
    """Verify tp_size>1 path: each rank's slice, with mocked all_reduce, matches full reference."""
    ...
    def test_tp2_slices_match_full(self):
        ...
        # Precompute global sum_sq — what a real all_reduce(SUM) would return.
        ...
        for rank in range(tp_size):
            ...
            # _norm is called twice per forward: first for q, then for k.
            # Return the precomputed global sum_sq each time.
            global_sums = iter([q_global_sum_sq, k_global_sum_sq])

            def _mock_all_reduce(tensor, **kwargs):
                return next(global_sums)
        ...
        self.assertTrue(
            ...
            f"TP assembled max diff {(assembled.float() - ref.float()).abs().max().item():.6f}",
        )
