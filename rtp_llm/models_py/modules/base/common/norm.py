...
# TODO: add fused kernel for LayerwiseQKRMSNorm
class LayerwiseQKRMSNorm(nn.Module):
    ...
    Pure-torch implementation — no device-specific kernels.
    ...
    @torch.no_grad()
    def _norm(
        self, x: torch.Tensor, weight: torch.Tensor, total_elts: int
    ) -> torch.Tensor:
        x_dtype = x.dtype
        x_f = x.float()
        local_sum_sq = x_f.pow(2).sum(dim=-1, keepdim=True)
        if self.tp_size > 1:
            from rtp_llm.models_py.distributed.collective_torch import Group, all_reduce

            local_sum_sq = all_reduce(local_sum_sq, group=Group.TP)
        var = local_sum_sq / float(total_elts)
        x_normed = x_f * torch.rsqrt(var + self.eps)
        return x_normed.to(x_dtype) * weight

    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        q, k, v = hidden_states.split([self.q_size, self.kv_size, self.kv_size], dim=-1)
        q_out = self._norm(q, self.q_weight, self.q_total_elts)
        k_out = self._norm(k, self.k_weight, self.kv_total_elts)
