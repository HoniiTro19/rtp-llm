    ...
    def forward(
        ...
        if input.dtype == torch.bfloat16:
            ...
            if cached is not None:
                # Producer-token cannot know the downstream linear's
                # ``scale_ue8m0`` (different linears in the same model may
                # disagree).  If the cached scale dtype mismatches this
                # linear's expectation, ignore the cache and recompute below.
        ...
        if not self._should_use_flashinfer(input):
            # Under torch.compile: do the bf16->fp8 quant HERE so it stays in
            # the same FX subgraph as the preceding activation (sigmoid*mul,
            # silu_and_mul, etc.).  The GEMM itself runs in a compile-disabled
            # wrapper that Dynamo treats as a graph-break boundary.
