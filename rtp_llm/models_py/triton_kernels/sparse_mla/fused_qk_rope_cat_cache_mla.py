...
def _fused_qk_rope_cat_cache_mla_bf16_kernel(
    ...
    if h_blk < H_TILES:
        ...
        if IS_NEOX:
            addrs_lo = q_base + h_offs[:, None] * stride_q_h + Q_ROPE_OFFSET + rh[None, :]
            addrs_hi = q_base + h_offs[:, None] * stride_q_h + Q_ROPE_OFFSET + HALF_ROPE + rh[None, :]
            ...
            addrs_e = q_base + h_offs[:, None] * stride_q_h + Q_ROPE_OFFSET + 2 * rh[None, :]
            addrs_o = addrs_e + 1
...
def _fused_qk_rope_cat_cache_mla_fp8_kernel(
    ...
    if h_blk < H_TILES:
        ...
        scale_base = page_idx * stride_kvc_fp32_page + slot_offset * stride_kvc_fp32_slot + (KV_LORA // 4)
        ...
        rope_bf16_base = page_idx * stride_kvc_bf16_page + slot_offset * stride_kvc_bf16_slot + (KV_LORA // 2) + (4 * 4 // 2)
        kpe_off = t * stride_kpe_t
        if IS_NEOX:
            k1 = tl.load(K_PE + kpe_off + rh).to(tl.float32)
            k2 = tl.load(K_PE + kpe_off + HALF_ROPE + rh).to(tl.float32)
...
def fused_qk_rope_cat_cache_mla(
    ...
    if kv_cache_type == "auto":
        ...
        _fused_qk_rope_cat_cache_mla_bf16_kernel[grid](
            q, k_pe, kv_cache,
            compressed_kv, slot_mapping, positions, cos_sin_cache,
