...
def baseline_rope(q, kpe, pos, csc):
    fi_rope._apply_rope_pos_ids_cos_sin_cache(
        q=q[..., NOPE_HEAD_DIM:], k=kpe.unsqueeze(1),
        q_rope=q[..., NOPE_HEAD_DIM:], k_rope=kpe.unsqueeze(1),
        cos_sin_cache=csc, pos_ids=pos, interleave=False,
    )
...
def fused_bf16(q, ck, kpe, kv_cache, slot, pos, csc):
    fused_qk_rope_cat_cache_mla(
        q, ck, kpe, kv_cache, slot, pos, csc,
        kv_lora_rank=KV_LORA, rope_head_dim=ROPE,
        is_neox_style=True, kv_cache_type="auto",
    )
...
def fused_fp8(q, ck, kpe, kv_cache, slot, pos, csc):
    fused_qk_rope_cat_cache_mla(
        q, ck, kpe, kv_cache, slot, pos, csc,
        kv_lora_rank=KV_LORA, rope_head_dim=ROPE,
        is_neox_style=True, kv_cache_type="fp8_ds_mla",
    )
...
def make_cos_sin_cache():
    inv = 1.0 / (10000.0 ** (torch.arange(0, ROPE, 2, device=device, dtype=torch.float32) / ROPE))
    positions = torch.arange(16384.0, device=device)
    return torch.cat([torch.outer(positions, inv).cos(),
                      torch.outer(positions, inv).sin()], dim=-1).to(torch.float32)


def make_inputs_bf16(T, num_heads=H, num_pages=8, seed=0):
    """Production BF16: q=[T,H,256], ck=[T,512], kpe=[T,64], kvc=bf16[N,64,576]."""
    torch.manual_seed(seed)
    q   = torch.randn(T, num_heads, QK_HEAD_DIM, device=device, dtype=torch.bfloat16).contiguous()
    ck  = torch.randn(T, KV_LORA, device=device, dtype=torch.bfloat16)
    kpe = torch.randn(T, ROPE, device=device, dtype=torch.bfloat16)
    kvc = torch.full((num_pages, BLOCK_SIZE, KV_CACHE_DIM), 7.5, device=device, dtype=torch.bfloat16)
...
def make_inputs_fp8(T, num_heads=H, num_pages=8, seed=0):
    ...
    ck  = torch.randn(T, KV_LORA, device=device, dtype=torch.bfloat16)
    kpe = torch.randn(T, ROPE, device=device, dtype=torch.bfloat16)
    kvc = torch.full((num_pages, BLOCK_SIZE, FP8_BYTES), 0xAA, device=device, dtype=torch.uint8)
    total_slots = num_pages * BLOCK_SIZE
    slot = torch.randperm(total_slots, device=device)[:T].to(torch.int64)
    pos  = torch.randint(0, 16383, (T,), device=device, dtype=torch.int32)
    return q, ck, kpe, kvc, slot, pos
...
def check_precision(name, make_fn, base_fn, fused_fn, cache_cmp_bytes=False):
    ...
    for seed in [0, 42, 12345]:
        for T in [1, 4, 32]:
            ...
            print(f"  seed={seed:>5} T={T:>2} H={H:>3}  "
...
bf16_ok = check_precision(
    "BF16 (auto) GLM-5 dims", make_inputs_bf16, baseline_bf16, fused_bf16,
    cache_cmp_bytes=False)

fp8_ok = check_precision(
    "FP8 (fp8_ds_mla) GLM-5 dims", make_inputs_fp8, baseline_fp8, fused_fp8,
...
def cudagraph_time(N, body_fn):
    ...
    torch.cuda.synchronize()
    return statistics.median([s.elapsed_time(e) * 1000.0 for s, e in zip(starts, ends)]) / N
