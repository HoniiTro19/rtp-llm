...
class MinimaxM2(QWenV2):
    ...
    def _patch_minimax_m2(cls, ckpt_path: str, config: ModelConfig) -> None:
        ...
        # ---- QK-norm ----
        # CausalAttention auto-dispatches per-head vs per-layer based on the
        # gamma tensor's numel(); we just need to flip qk_norm on so the
        # weight loader registers the gamma slots.
        config.qk_norm = bool(cj.get("use_qk_norm", False))
        qk_norm_type = cj.get("qk_norm_type", "per_head")
        ...
        eos = gen_cfg.get("eos_token_id", cj.get("eos_token_id"))
        if eos is not None:
            # Some configs store eos as a list; pick the first.
            if isinstance(eos, list):
                eos = eos[0]
            config.special_tokens.eos_token_id = eos

        # ---- Stop words ----
        eot_id = _lookup_added_token_id("[e~[")
        if eot_id is not None:
            config.special_tokens.stop_words_id_list = [[eot_id]]
        else:
